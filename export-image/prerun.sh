...
parted --script "${IMG_FILE}" unit B mkpart primary xfs "${ROOT_PART_START}" "$((ROOT_PART_START + ROOT_PART_SIZE - 1))"
...
mkfs.xfs -L rootfs "$ROOT_DEV" > /dev/null

mount -v "$ROOT_DEV" "${ROOTFS_DIR}" -t xfs
